...
class ConvMixin(object):
  ...
  def conv_op(cls, node, **kwargs):
    strides = cls.caffe2onnx_param(list(node.convolution_param.stride))
    dilations = cls.caffe2onnx_param(list(node.convolution_param.dilation))
    kernel_shape = list(node.convolution_param.kernel_size) * 2
    n_groups = node.convolution_param.group

    pads = cls.caffe2onnx_param(list(node.convolution_param.pad))
  ...
  @staticmethod
  def caffe2onnx_param(caffe_param):
